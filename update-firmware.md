...
By default, FortiGate devices have a global HA configuration parameter `route-ttl`, which is set to 10 seconds in version 7.2. This parameter determines how long the new primary unit retains routes during a failover that are synchronized with it. Because of that setting, the new primary has only 10 seconds to reestablish BGP as well as relearn and advertise routes/reconverge. 10 seconds is not enough time for that on most FortiGates that are setup with dynamic routing such as BGP.
...
* Set graceful-update-delay to 20. This reduces the maximum amount of time that the FortiGate will wait for all GR capable peers to send end-of-rib marker before doing route selection.
